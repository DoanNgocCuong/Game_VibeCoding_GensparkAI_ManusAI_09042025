...
export const calculateXPProgress = (baseXP: number, streakDays: number = 0): XPProgress => {
  let level = 0;
  let totalXP = baseXP;
...
export const calculateXPDeduction = (totalXP: number, streakDays: number, consecutiveMissedDays: number = 1): number => {
  ...
  const severityMultiplier = consecutiveMissedDays >= 8 ? 3.0 :
                            consecutiveMissedDays >= 4 ? 2.0 :
                            consecutiveMissedDays >= 3 ? 1.5 :
                            consecutiveMissedDays >= 2 ? 1.2 : 1.0;
